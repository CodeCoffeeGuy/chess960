...
// POST /api/study/[id]/comment - Add a comment to a study
export async function POST(
  ...
  try {
    ...
    // Check if study exists and is accessible
    ...
    // Check if user can comment (must be public or owner)
    ...
    const comment = await (prisma as any).studyComment.create({
      data: {
        studyId: id,
        userId: session.user.id,
        text: text.trim(),
        movePly: movePly || null,
      },
      include: {
        user: {
          select: {
